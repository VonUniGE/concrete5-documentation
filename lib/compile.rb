...
def process(which)
    ...
    f = File.open($rootDir + "/output/" + which + "-docbook.xml", "wb")
    f.write(docbook)
    f.close
    print "done.\n"
    ...
    if system("xsltproc --output " + $rootDir + "/output/" + which + "/myfile.html " + $rootDir + "/lib/html-chunked-parameters.xsl " + $rootDir + "/output/" + which + "-docbook.xml") == false
