...
Let's look at how these database tables are managed.
(**Note**: I'm ommitting certain bits of code that, while important for the survey block as it works in Concrete5, don't necessarily assist in the understanding of how this works for the average block developer).
...
Delete the page and still see the block? That's because the page exists in your site's trash can.
